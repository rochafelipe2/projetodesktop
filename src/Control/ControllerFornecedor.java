...
public class ControllerFornecedor extends ControllerArquivoBinario {
	...
	public int buscarUltimoId() throws IOException{
		...
		try {
			...
			if(index > 0){
				lastId = ((FornecedorModel)this.objetos.get(index-1)).id+1;
			}
		} catch (Exception e) {
	...
	public void atualizar(FornecedorModel model) {
		...
		FornecedorModel edited = (FornecedorModel) objetos.get(model.id);
		edited = model;
		escrever(false);
	...
	public void remover(int id) {
		...
		FornecedorModel edited = (FornecedorModel) objetos.get(id-1);
